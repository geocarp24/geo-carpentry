 ...
 * EXCEPT on service-city programmatic SEO pages (those emit their own
 * page-specific schema with city-targeted areaServed, see below).
 *
 * Hooked early so SEO plugins can still override if needed.
 */
add_action( 'wp_head', function () {
    // Skip global schema on service-city landing pages — they emit a city-specific one
    if ( class_exists( 'Geo_Service_City_Rewriter' ) && Geo_Service_City_Rewriter::is_service_city_page() ) {
        return;
    }
...
/**
 * Service-City landing pages: emit city-specific GeneralContractor schema.
 * Replaces the global areaServed-all-17-cities schema with one focused on the
 * specific city for the current page. Better local SEO signal.
 *
 * Triggered only on pages with _gc_service_slug + _gc_city_slug postmeta
 * (created by Claude Code WP-CLI bulk creator from Content_Queue).
 *
 * Added 2026-05-27 by Claude Code (Bucket B4).
 */
add_action('wp_head', function () {
    if (!class_exists('Geo_Service_City_Rewriter')) return;
    if (!Geo_Service_City_Rewriter::is_service_city_page()) return;

    $post_id      = get_the_ID();
    $service_slug = get_post_meta($post_id, '_gc_service_slug', true);
    $service_name = get_post_meta($post_id, '_gc_service_name', true);
    $city_slug    = get_post_meta($post_id, '_gc_city_slug', true);
    ...
    $schema = [
        ...
        'name'        => 'Geo Carpentry LLC',
        'description' => sprintf(
            '%s in %s, WI by Geo Carpentry LLC. Licensed general contractor serving Northeast Wisconsin since 2014. Free estimate in 24 hours.',
            $service_name,
            $city_name
        ),
        'url'         => get_permalink(),
        ...
        // CITY-SPECIFIC areaServed — the whole point of this override
        'areaServed'  => [
            '@type'           => 'City',
            'name'            => $city_name,
            'addressRegion'   => 'WI',
    ...
    // Carry over aggregateRating from global schema if present (set by Jorge in WPCode)
    // This is a soft reference — if WPCode hasn't injected one, this won't add fake data.
...
/**
 * Service-City landing pages: emit FAQPage schema if FAQ data is present in postmeta.
 * Bulk creator (Bucket B5) populates _gc_faq_jsonld as JSON-encoded array of {q, a}.
 *
 * Added 2026-05-27 by Claude Code (Bucket B4).
 */
add_action('wp_head', function () {
