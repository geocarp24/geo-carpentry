<?php
// ============================================================
// GEO AI AGENT v4 — Geo Carpentry LLC
// SMS webhook receiver for incoming leads.
//
// v4 changes (2026-07-30) — "Smart Scheduling":
//   - Before asking Claude what to say, the agent now pulls every OTHER
//     lead's confirmed "Appointment Date" from Airtable (next 14 days)
//     and hands that list to Claude as "already booked" so two clients
//     can never be double-booked into the same Mon/Wed/Thu 4-6pm slot.
//   - Works together with geo_claude_v3.php, which restricts GEO to only
//     ever offering Monday/Wednesday/Thursday, 4:00-6:00 PM CT.
//   - A confirmed appointment created here is picked up by the new "Cal"
//     VPS agent (see HANDOFF_CAL_AGENT_VPS.md) which mirrors it onto
//     Jorge's real Google Calendar (admin@geocarpentry.com) every 15 min.
//
...
//   "message.sent" / "message.delivered" webhook event ENABLED for the
//   pause feature to work (confirmed present in Events Log 2026-07-30).
//
// Flow: SMS received → dedup → Airtable lookup/upsert
//       → pause check → fetch booked slots → conversation history
//       → Claude reply → SMS send → Telegram escalation if hot lead
//       → CRM update → log
// ============================================================
...
// ── v4: Smart-scheduling helper — fetch already-booked appointment slots ──
// Self-contained Airtable GET (doesn't rely on geo_airtable.php internals,
// same pattern already used in geo_seguimiento.php).
function geo_agent_fetch_booked_slots(?string $excludePhone = null): array {
    if (!defined('AIRTABLE_TOKEN') || !defined('GEO_AT_BASE') || !defined('GEO_AT_LEADS')) {
        return [];
    }
    $now      = new DateTime('now', new DateTimeZone('America/Chicago'));
    $twoWeeks = (clone $now)->modify('+14 days');
    $filterFormula = "AND({Lead Status}='Appointment Set',{Appointment Date}!='')";
    $url = 'https://api.airtable.com/v0/' . GEO_AT_BASE . '/' . GEO_AT_LEADS
         . '?filterByFormula=' . urlencode($filterFormula)
         . '&fields[]=Phone&fields[]=Appointment+Date&fields[]=Full+Name';
    ...
    if ($code !== 200 || !$body) {
        geo_log_warn('booked_slots_fetch_failed', ['code' => $code]);
        return []; // fail open — Claude just won't have the list, prompt rules still apply
    }
    ...
    foreach ($data['records'] ?? [] as $rec) {
        $f = $rec['fields'] ?? [];
        $phone = $f['Phone'] ?? '';
        if ($excludePhone && $phone === $excludePhone) continue; // don't count the lead's own appointment
        $apptStr = $f['Appointment Date'] ?? null;
        if (!$apptStr) continue;
        try {
            $dt = new DateTime($apptStr, new DateTimeZone('America/Chicago'));
        } catch (Exception $e) { continue; }
        if ($dt < $now || $dt > $twoWeeks) continue; // only near-term slots matter
        $slots[] = $dt->format('D Y-m-d g:i A') . ' T';
    }
    return $slots;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && empty($_GET)) {
    echo json_encode(['ok' => true, 'service' => 'geo_agent', 'version' => 4]);
    exit;
}
...
// ── 6b. v4: Fetch already-booked slots so GEO never double-books ────
$bookedSlots = geo_agent_fetch_booked_slots($fromPhone);
...
// ── 12. Log conversation to Airtable (QC) — DISABLED v4 ───
// Removed 2026-07-30 to cut Airtable API usage (was 2 writes/message,
// doubling request volume with zero functional benefit — full history
// already lives in local JSON via geo_conv_append_turn, step 10).
// Re-enable only if Jorge specifically wants conversation QC visible
// inside Airtable itself.
