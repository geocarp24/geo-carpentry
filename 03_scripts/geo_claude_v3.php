<?php
// ============================================================
// lib/geo_claude.php — GEO AI Agent v3
// System prompt + Claude API call for GEO Carpentry SMS agent
//
// v3 changes (2026-07-30):
//   - NEW: Appointment availability restricted to Mon/Wed/Thu, 4:00-6:00 PM CT ONLY.
//     GEO will never offer any other day or time — if asked, it explains
//     the fixed schedule and offers the nearest valid slot.
//   - NEW: Accepts $bookedSlots (already-confirmed appointments from Airtable)
//     so GEO never double-books two clients into the same 1-hour slot.
//   - Appointments are 1 hour long → max 2 slots/day (4-5pm, 5-6pm) × 3 days = 6/week.
//
 ...
 *
 * @param array $ctx  Context from geo_agent.php. New key: 'bookedSlots' —
 *                     array of human-readable strings describing slots
 *                     already taken, e.g. ["Wed 2026-08-06 4:00 PM", ...]
...
function geo_claude_decide(array $ctx): array {
    ...
    $bookedSummary = empty($bookedSlots)
        ? "None — every Mon/Wed/Thu 4-6pm slot this week and next is currently open."
        : implode('; ', $bookedSlots);
...
## 📅 APPOINTMENT AVAILABILITY — STRICT RULES (v3)
Jorge ONLY does in-person estimates on these days and times. This is NON-NEGOTIABLE —
never offer, confirm, or accept any day/time outside this window, even if the client asks:

**Available: Monday, Wednesday, Thursday — 4:00 PM to 6:00 PM Central Time ONLY.**
Each appointment is 1 hour, so there are exactly two slots per available day:
  - 4:00 PM – 5:00 PM
  - 5:00 PM – 6:00 PM

**Already booked (do NOT offer these — they are taken):**
{$bookedSummary}

Rules for offering times:
- Offer a FREE estimate only after you have: name + service + address
- Only ever propose Mon, Wed, or Thu, and only 4:00 PM or 5:00 PM as the start time
- Skip any slot listed as already booked above — offer the next open one instead
- If the client asks for a different day/time (e.g. "can you do Saturday?" or "what about mornings?"),
  politely explain: "Jorge does free estimates Mondays, Wednesdays, and Thursdays from 4-6pm —
  I can get you set up for [nearest available slot]. Does that work?"
- Always confirm with an exact date + start time before setting appointmentDate
- appointmentDate in your JSON response MUST fall on a Monday, Wednesday, or Thursday,
  with a time of exactly 16:00 or 17:00 (Central Time), or it must be null
...
```json
{
  ...
  "appointmentDate": "ISO8601 datetime (Mon/Wed/Thu, 16:00 or 17:00 CT) if confirmed, null if not",
...
- NEVER confirm an appointmentDate outside Mon/Wed/Thu 4-6pm CT — see rules above
PROMPT;
    ...
    // ── v3 SAFETY NET: never let an out-of-window appointment slip through ──
    // Even though the prompt forbids it, we double-check here in code so a
    // model mistake can never actually book the wrong day/time.
    if (!empty($geo['appointmentDate'])) {
        try {
            $apptDt = new DateTime($geo['appointmentDate'], new DateTimeZone('America/Chicago'));
            $dow    = (int) $apptDt->format('N'); // 1=Mon ... 7=Sun
            $hour   = (int) $apptDt->format('G');
            $isValidDay  = in_array($dow, [1, 3, 4]); // Mon, Wed, Thu
            $isValidHour = in_array($hour, [16, 17]); // 4pm or 5pm start
            if (!$isValidDay || !$isValidHour) {
                geo_log_warn('appointment_out_of_window_blocked', [
                    'proposed' => $geo['appointmentDate'],
                ]);
                $geo['appointmentDate'] = null;
                $geo['newStatus'] = $geo['newStatus'] === 'Appointment Set' ? 'Qualified' : $geo['newStatus'];
                // Nudge the client back to a valid slot instead of silently failing
                $geo['responseToClient'] = "Oops, let me fix that — Jorge only does free estimates Mondays, Wednesdays, and Thursdays from 4-6pm. What day works best for you in that window?";
            }
        } catch (Exception $e) {
            geo_log_warn('appointment_date_parse_failed', ['raw' => $geo['appointmentDate']]);
            $geo['appointmentDate'] = null;
        }
    }
