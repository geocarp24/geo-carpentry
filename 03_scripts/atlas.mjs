...
const SUPERVISOR_STATE_PATH = '/opt/alex-bot/agents/supervisor/last_run_geo.json';
...
async function triageIssues(issues) {
  ...
  for (const issue of issues) {
    ...
    if (!matched) {
      console.log(`[Atlas] No playbook match — escalating raw issue`);
      escalations.push({ id: 'unknown', message: `⚠️ Issue sin playbook: <code>${issueStr.slice(0, 200)}</code>` });
    }
  }
